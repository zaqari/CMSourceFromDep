...
Train_Data2 = '/Users/ZaqRosen/Documents/Corpora/mohler/train_data11-9v4.csv'
Test_Data2 = '/Users/ZaqRosen/Documents/Corpora/mohler/test_data11-9v4.csv'
...
nD = 13
drop_out_x = .42
hd_units=[len(nClasses)]
num_tr_steps=14573
model_dir = [
        #'/Users/ZaqRosen/kGEN_model/kGEN-ex18-49',
        '/tmp/kGEN/',
        #tempfile.mkdtemp()
        ]
...
CATEGORICAL_COLUMNS = ['LmTarget', 'subj', 'dobj', 'syn', 'verb', 'obl1', 'obl2']
...
tref = tf.contrib.layers.sparse_column_with_hash_bucket("LmTarget", hash_bucket_size=int(1000))
...
#       EX21.2  Dimensions=13D          .804 acc        dnn_units=[77]  dropout=.4      steps=31000
...
deep_columns = [
        ...
        tf.contrib.layers.embedding_column(trefxobl2, dimension=nD),
        tf.contrib.layers.embedding_column(trefxverbxdobj, dimension=nD),
        tf.contrib.layers.embedding_column(trefxverbxsubj, dimension=nD),
        tf.contrib.layers.embedding_column(trefxobl1xobl2, dimension=nD),
...
#####
def train_model(dropout,  train_steps=int(len(df_train)*18), calc_results=True, resultsteps=10):
...
##TRAIN & TEST MODEL##
x=train_model(drop_out_x, num_tr_steps)
print(x)

#m.fit(input_fn=train_input_fn, steps=30856)
